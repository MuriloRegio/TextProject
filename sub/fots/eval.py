...
def predict(filename, model, with_image, output_img_dir, with_gpu, labels, output_txt_dir):
	return Toolbox.predict(filename, model, with_image, output_img_dir, with_gpu, labels, output_txt_dir)

def process(filelist, status, model, with_image=False, output_img_dir=False, with_gpu=False, labels=False, output_txt_dir=False):
	i = 0
	for filename in filelist:
		polys = Toolbox.predict(filename, model, with_image, output_img_dir, with_gpu, labels, output_txt_dir)
		if polys is None:
			polys = np.asarray([])
	...
	status["done"]=1
	# return list(polys.astype(np.int32))
